...
<form action="{{ route('categories.update', $category->id) }}" method="POST" class="mt-3">
    ...
    <div class="mb-3 mt-3">
        ...
        <input type="text" id="name" class="form-control @error('name') border-danger @enderror" placeholder="Enter category name" name="name" value="{{ old('name', $category->name) }}" required autofocus>
    </div>
